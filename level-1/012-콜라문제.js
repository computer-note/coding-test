...
function solution(a, b, n) {
	...
	let currentEmptyBottleNum = n;
	let totalNewBottleNum = 0;
	while(currentEmptyBottleNum >= requiredEmptyBottleNum) {
		//마트에서 빈 병으로 교환 받은 새 병의 개수 계산
		const acquiredNewBottleNum = Math.trunc(currentEmptyBottleNum / requiredEmptyBottleNum) * rewardedNewBottleNum;

		//총 획득한 새 병의 개수 업데이트
		totalNewBottleNum += acquiredNewBottleNum;

		//교환 후 남은 빈 병의 개수 계산
		currentEmptyBottleNum %= requiredEmptyBottleNum;

		//현재 반복에서 획득한 새 병이 소비되어 빈 병이 됨
		const newEmptyBottleNum = acquiredNewBottleNum;

		//총 빈 병의 개수 업데이트
		currentEmptyBottleNum += newEmptyBottleNum;
	}

	return totalNewBottleNum;
}
...
받은 1병과 5병을 받았을 때 남은 1병을 모두 마신 뒤 가져가면 
1병을 또 받을 수 있습니다. 
