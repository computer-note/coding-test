function solution(k, score) {
	...
	const kthScores = [];
	const topKScoresOfNthDay = [];
	for (let nthDay = 1; nthDay <= totalDay; ++nthDay) {
		topKScoresOfNthDay.push(score[nthDay - 1]);
		topKScoresOfNthDay.sort((a, b) => b - a);
		topKScoresOfNthDay.splice(k);

		if (nthDay < k) {
			kthScores.push(topKScoresOfNthDay[nthDay - 1]);
		} else {
			kthScores.push(topKScoresOfNthDay[k - 1]);
		}
	}
