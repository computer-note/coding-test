function mySolution(k, m, score) {
   const descSortedScore = score.sort((a, b) => b - a);
   const appleBoxes = [];
   ...
   for (let i = 0; i + m <= score.length; i += m) {
      const appleBox = descSortedScore.slice(i, i + m);
      appleBoxes.push(appleBox);
   }

   const income = appleBoxes.reduce((income, curBox) => {
      return income + curBox.at(-1) * m;
   }, 0);
